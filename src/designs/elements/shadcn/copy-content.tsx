  ...
  "inline-flex items-center justify-center cursor-pointer rounded-md transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        ...
        secondary: "bg-white/7.5 text-[oklch(0.985_0_0)] shadow-xs",
      ...
      size: {
        ...
        text: "h-8 [&_svg]:size-4 w-auto rounded-md gap-x-2 px-2",
      },
    },
    defaultVariants: {
      variant: "default",
...
function CopyContent({ content, className, size, variant, delay = 3000, onClick, onCopy, isCopied, onCopyChange, ...props }: CopyContentProps) {
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait">
        ...
        >
          <HugeiconsIcon icon={localIsCopied ? Tick01Icon : Copy01Icon} className="text-[oklch(0.985_0_0)]" />
        </m.span>
      </AnimatePresence>
      {size === "text" && <span className="text-[0.8rem] whitespace-nowrap text-[oklch(0.985_0_0)] select-none">Copy code</span>}
    </m.button>
  );
}

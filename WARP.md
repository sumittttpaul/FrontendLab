...
> **See also:** [AGENTS.md](./AGENTS.md) for full project guidelines.

## Context for Warp Terminal

You are in the **SumitApp** repository, which contains a CLI tool, a documentation site, and project templates.

**Key Locations:**

- `create-sumit-app/` -> The CLI source code.
- `official-site/` -> The documentation website.
- `templates/default/` -> The blueprint for new user projects.
...
cd official-site
bun install
bun dev            # Turbo dev server
bun build          # Prod build
bun start
```

### templates/default (The Template)

```bash
# To test the template as users see it:
npx create-sumit-app test-app
cd test-app
bun dev
```
